...
\author{[Author Names]\\
\small [Institution] \quad \texttt{[email]}}
...
How entanglement builds up in parameterised quantum circuits is a central
question in quantum information theory and near-term quantum computing.
For Clifford circuits, stabiliser formalism provides an efficient classical
description~\cite{gottesman1997stabilizer,aaronson2004improved}, but the
\emph{analytic dependence} of von Neumann entropy on continuous rotation
angles has received limited systematic treatment.

We study a canonical family: product $R_x(\theta_i)|0\rangle$ states on
subsystem $A$, coupled to $|0\rangle^{\otimes n_B}$ on subsystem $B$ by a
single layer of CNOT gates encoded in a binary matrix $M$.  The central
observation is that the reduced state $\rho_B$ is always diagonal in the
computational basis, with eigenvalues determined entirely by the $\F_2$-linear
image of $M$ and the squared cosines $\cos^2(\theta_i/2)$.  A
Walsh–Hadamard transform then yields a closed-form entropy formula.

The rank of $M$ over $\F_2$ plays the role of the maximum achievable entropy;
this bound is tight and attained at $\theta_i = \pi/2$.  The formula
extends naturally to multi-layer circuits via $\F_2$-addition of connectivity
matrices, to arbitrary pure inputs via a pushforward of the probability
weights, and to diagonal mixed states.

\subsection*{Related work}

The connection between stabiliser circuits and $\F_2$ linear algebra is
classical~\cite{gottesman1997stabilizer}.  The present work differs in
providing \emph{exact, parameter-dependent} entropy formulas rather than
membership in the stabiliser group.  Entanglement in parameterised circuits
has been studied in the context of expressibility~\cite{sim2019expressibility}
and barren plateaus~\cite{mcclean2018barren}; the Walsh–Hadamard approach
here gives analytic control absent in those statistical treatments.
...
state-vector computation when $n_A \gg n_B$.
\end{remark}
...
available at \url{https://github.com/[username]/qse}.
